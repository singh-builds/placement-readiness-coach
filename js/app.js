...
function renderShell({ active, title, subhead }) {
  ...
  const account = typeof session === "function" ? session() : null;
  const displayName = account?.name || state.student.name;
  ...
  topbar.innerHTML = `
    ...
    <div class="d-flex align-items-center gap-2">
      ...
      <div class="user-chip">
        ...
        <div>
          <div style="font-weight:600; line-height:1.1">${displayName}</div>
          <div style="color:var(--slate); font-size:0.72rem;">${account?.role?.replaceAll("_", " ") || state.student.roll}</div>
        </div>
        <button class="logout-button" type="button" title="Sign out" aria-label="Sign out" onclick="signOut()"><i class="bi bi-box-arrow-right"></i></button>
      </div>
    </div>
  `;
